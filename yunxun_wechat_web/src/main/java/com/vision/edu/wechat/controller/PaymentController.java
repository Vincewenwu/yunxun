package com.vision.edu.wechat.controller;
...
import com.vision.edu.wechat.util.ReturnModel;
import com.vision.edu.wechat.util.Sha1Util;
import com.vision.edu.wechat.util.XMLUtil;
...
public class PaymentController extends GenericController {
	...
	public void getPrepayId(HttpServletResponse response, HttpServletRequest request) throws WxErrorException, WxPayException {
		WxPayUnifiedOrderRequest payInfo = WxPayUnifiedOrderRequest.newBuilder().openid(request.getParameter("openid")).outTradeNo(request.getParameter("out_trade_no")).totalFee(Integer.valueOf(request.getParameter("total_fee"))).body(request.getParameter("body")).tradeType(request.getParameter("trade_type")).spbillCreateIp(request.getParameter("spbill_create_ip")).notifyURL("").build();
		this.logger.info("PartnerKey is :" + this.payConfig.getMchKey());
